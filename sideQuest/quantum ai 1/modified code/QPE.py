...
class quantum_phase_estimation:
    ...
    def simulate_circuit(self,circ):
        sim = cirq.Simulator(split_untangled_states=False)  # change
...
if __name__ == '__main__':
    import os, contextlib  # change
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'modified outputs')  # change
    os.makedirs(out_dir, exist_ok=True)  # change
    with open(os.path.join(out_dir, '(8)_(QPE)_(4+8qb).txt'), 'w') as f, contextlib.redirect_stdout(f):  # change
        for n in [4, 8]:  # change
            print(f'\n===== QPE with {n} ancilla qubits =====')  # change
            main(num_ancillia_qubits=n)  # change
    print('wrote (8)_(QPE)_(4+8qb).txt')  # change