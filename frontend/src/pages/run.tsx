...
function Run({ connected, send_msg, messages = [] }: RunProps) {
    ...
    // don't leave an interval running if we navigate away mid-cooldown
    useEffect(() => () => {
    ...
    const run = () => {
        const now = Date.now()
        if (now < next_allowed.current) return   // synchronous, beats double-dispatch
